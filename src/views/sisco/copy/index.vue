...
<script>
import { listCopy, getCopy, delCopy, addCopy, updateCopy } from "@/api/sisco/copy";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listCopy(this.queryParams).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.stageId != null) {
            updateCopy(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addCopy(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('sisco/copy/export', {
        ...this.queryParams
